...
if (usingRequire) {
    ...
    // TODO: We should min/uglify the js files?
    copyCommand = new rsync();
    copyCommand.flags('aqu')
        .shell('ssh')
        .source(sourceFolder + '/js')
        .destination(publicBuildFolder);

    copyCommand.execute(function(error, code, cmd) {
        if (error) {
            console.log('*** Not able to copy js folder', cmd, error.message);
        } else {
            console.log('Finished copying js', cmd);
        }
    });
}
